...
from spoc_utils.bbox_utils import get_basis_for_3d_box
from spoc_utils.data_generation_utils.loc_grid_conversion import locs2grids, grids2locs
...
def is_any_object_sufficiently_visible_and_in_center_frame(
    ...
    # and the minimum to be 100 (the old one for SMALL_TYPES).
    # Note: Using default scale=15,000 instead of 10,000 as `ProportionOfObjectVisible` resolves it.
    # Note: default image height = 240 -> 240 ^ 2 = 57600 pixels is the area for a 1:1 image shape ratio.
    if alignment_threshold is None:
        alignment_threshold = ALIGNMENT_THRESHOLD
    scale_to_apply = scale * (controller.navigation_camera.shape[0] ** 2) / 57600
    ...
    for oid in object_ids:
        ...
        if alignment <= alignment_threshold:
            ...
            try:
                ...
                color = controller.controller.last_event.object_id_to_color[oid]
                pixel_mass = (oid_to_seg == color).sum() / 3  # each pixel has 3 channels
                empty_top = (
